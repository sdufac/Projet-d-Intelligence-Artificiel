...
def interactive_view(G_dict, start_a, start_b,
    ...
    while True:
        legal = get_legal_moves(game.state, game.G, current)
        if not legal:
            winner = 1 - current
            break

        print(f"Player {current}'s turn. Legal moves: {legal}")
        move = strategies[current].select_move(game.state, game.G, current)
    ...
    # Game over
    draw_state(G, game, pos, ax, title=f"Game Over – Winner: Player {winner}")
